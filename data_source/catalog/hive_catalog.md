...
| aws.glue.iam_role_arn         | 否       | 有权限访问 AWS Glue Data Catalog 的 IAM Role 的 ARN。采用 Assumed Role 鉴权方式访问 AWS Glue 时，必须指定此参数。这样，StarRocks 在使用 Hive Catalog 访问 Hive 数据时将会担任该 IAM Role。 |
| aws.glue.region               | 是       | AWS Glue Data Catalog 所在的地域。示例：`us-west-1`。        |
| aws.glue.access_key           | 否       | IAM User 的 Access Key。采用 IAM User 鉴权方式访问 AWS Glue 时，必须指定此参数。这样，StarRocks 在使用 Hive Catalog 访问 Hive 数据时将会担任该 IAM Role。 |
| aws.glue.secret_key           | 否       | IAM User 的 Secret Key。采用 IAM User 鉴权方式访问 AWS Glue 时，必须指定此参数。这样，StarRocks 在使用 Hive Catalog 访问 Hive 数据时将会担任该 IAM Role。 |
